...
# Target-ligand data
1. Determine the 1st and 2nd principal components on the target-ligand data and visualise their loadings on the variables.

First transform data so it looks more normal.
```{r}
df <- readRDS("../data/data-driven-drug-discovery/target_ligand_joined.rds")
# examine histograms
df %>% 
  select(-one_of("target", "chembl_id")) %>% 
  mutate(id=seq_along(pchembl_value)) %>% 
  melt(id.vars="id") %>% 
  ggplot(aes(x=value)) +
  geom_histogram() +
  facet_wrap(~variable, scales="free")

# looks like molwt is non-normal; rotable bonds is non-normal; molMR is non-normal; TPSA same: for those non-zero ones take log; others sqrt
df_kept <- df %>%
  select(MolWt, MolMR) %>% 
  mutate_all(log)
...
# qed looks right-skewed: take square root
df <- df %>%
  mutate(qed=sqrt(qed),
         TPSA=sqrt(TPSA),
         NumRotatableBonds=sqrt(NumRotatableBonds))

df <- df %>% 
  select(-one_of("MolWt", "MolMR")) %>% 
  bind_cols(df_kept)
...
x <- df %>% 
  ...
  filter(!is.infinite(MaxAbsPartialCharge),
         !is.infinite(MinAbsPartialCharge))
...
2. Plot the 1st and 2nd PCA scores for each ligand: does this help us to separate out the targets?
```{r}
df_temp <- df %>% 
  cbind(pca$x)

df_temp %>% 
  ggplot(aes(x=PC1, y=PC2, colour=as.factor(target))) +
  geom_point()
```

3. Do higher order PCs help to separate these?
```{r}
df_temp %>% 
  ggplot(aes(x=PC3, y=PC4, colour=as.factor(target))) +
  geom_point()
```

4. (optional) examine the component loadings using a sparse method
```{r}
# here alpha controls the sparcity (high values correspond to more sparse)
fit <- sparsepca::spca(x, verbose=F, alpha=0.0002)
loadings <- fit$loadings %>% 
  as.data.frame() %>% 
  mutate(rowname=colnames(x))

loadings %>%
  select(V1, V2, rowname) %>%
  melt(id.vars="rowname") %>%
  ggplot(aes(x=rowname, y=value)) +
  geom_col() +
  coord_flip() +
